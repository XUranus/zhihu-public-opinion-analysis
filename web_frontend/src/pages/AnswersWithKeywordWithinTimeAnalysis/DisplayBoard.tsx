...
import { Avatar, List ,Card, Row, Col, Comment, Statistic, Button, Typography, Tabs} from 'antd'
import {UserOutlined} from '@ant-design/icons'
import {DonutChart, PieChart, BarChart, LineChart, RoseChart,OverlappedComboChart} from '@opd/g2plot-react'

import WordCloud from '@/components/WordCloud'
import {AnswersWithKeywordWithinTimeAnalysisResultType} from '@/models/analysisWithKeywordWithinTime'
...
const DisplayBoard = (props: AnswersWithKeywordWithinTimeAnalysisProps) => {
          ...
          <Row>
            <Col span={12}>  
              <Card style={{marginRight:5,marginLeft:5}} title={"词云"}>
                <WordCloud 
                  height={300}
                  words={relatedKeywords.map((pair,index) => ({
                    word: pair.word,
                    weight: pair.count,
                    id: index
                  }))}
            ...
            <Col span={6}>
              <Card style={{marginRight:5,marginLeft:5}} title={"热门词汇"}> 
                <Row>
                  <Col span={12}>
                    <List dataSource={relatedKeywords.slice(0,5)} renderItem={(data)=>(<List.Item><span style={{fontSize:22}}>{data.word}&nbsp;<span style={{fontSize:10,color:'red'}}>{data.count}</span></span></List.Item>)}/>
                  </Col>
                  <Col span={12}>
                    <List dataSource={relatedKeywords.slice(5,10)} renderItem={(data)=>(<List.Item><span style={{fontSize:22}}>{data.word}&nbsp;<span style={{fontSize:10,color:'red'}}>{data.count}</span></span></List.Item>)}/>
                  </Col>
                </Row>
              </Card>
            </Col>
                  ...
                  layers={[
                    {
                      ...
                      smooth:true,
                      name: '实际每日增长',
                      data: heatTrend.dayTrend.slice(0,10),
                      xField: 'time',
                      yField: 'value',
                    },{
                      type: 'line',
                      name: '预测每日增长',
                      data: heatTrend.dayPredict.slice(0,10),
                      xField: 'time',
                      yField: 'value',
                      color: '#f8ca45',
                      smooth: true
